...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://i.ibb.co/Tb9WdkC/file.jpg'
                    let ppgp = 'https://i.ibb.co/Tb9WdkC/file.jpg'
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Bienvenido, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Desconocido') :
                            ...
                            let wel = API('fgmods', '/api/welcome', {
                                ...
                                profile: pp,
                                background: 'https://i.ibb.co/mHG3Xvs/file.jpg'
                            }, 'apikey')
        ...
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://i.ibb.co/jHctydb/Genesis-Bot.jpg') 
            if (!text)
